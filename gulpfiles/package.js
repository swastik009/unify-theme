...
function fileInclude() {
  return gulp
    ...
    .pipe(replace(new RegExp('(\\.+\\/)+' + "node_modules" + '\/.*\\.*', 'g'), function (match, p1) {
      ...
      return match.replace('node_modules', "./unify/dist/assets/vendor".replace("./unify/dist" + '/', ''))
    }))
    .pipe(deleteLines({
      'filters': [
...
function minCSS() {
  return gulp
    ...
    .pipe(gulp.dest("./unify/dist/assets/css"))
}
...
function minJS() {
  ...
  if (bundle.length) {
    return gulp
      ...
      .pipe(uglify())
      .pipe(gulp.dest("./unify/dist/assets/js"));
  } else {
...
function copy() {
  return gulp
    ...
    .pipe(gulp.dest("./unify/dist/assets"))
}
...
function copyNode() {
  ...
  if ([...node].length) {
    return gulp
      .src([...node])
      .pipe(gulp.dest("./unify/dist/assets/vendor"));
  } else {
    return new Promise(function (resolve, reject) {
      resolve();
    });
...
function copyDependencies() {
  for (var k in config.copyDependencies.dist) {
    ...
    gulp
      .src(path)
      .pipe(gulp.dest("./unify/dist" + '/' + config.copyDependencies.dist[k]))
  }
